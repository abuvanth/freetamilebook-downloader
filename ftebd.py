...
for atype in e.findall('book'):
    ...
    if atype.find('pdf').text is not None:
       ...
       data=requests.get(atype.find('epub').text, stream=True,headers=header)
       with open(atype.find('link').text.rsplit('/',2)[1]+".epub",'wb') as fil:
           fil.write(data.content)  
